...
export default class Quake {
  ...
  async getEarthQuakesByRadius(position, radius = 100) {
    ...
    let url = `https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=2019-01-01&endtime=2019-03-02&latitude=${position.lat}&longitude=${position.lon}&maxradiuskm=100`
    console.log('url: ' + url)
    await getJSON(url).then(result => {
      this._quakes = result.features;
    })
